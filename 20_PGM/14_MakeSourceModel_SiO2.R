...
##Setting
all.data.x <- read.csv("../10_Data/inorganic_descriptor.csv", header=TRUE, stringsAsFactors=FALSE)
all.data.y <- read.csv("../10_Data/inorganic_property.csv",   header=TRUE, stringsAsFactors=FALSE)
rdf.data.x <- read.csv("../10_Data/pdf.csv", header=TRUE, stringsAsFactors=FALSE)


source.y.name      <- "AllSiO2"
source.id          <- all.data.x[,1]!="SiO2" #"SiO2","CdI2","ZnS" 
source.y           <- all.data.y[source.id, 10]
source.x           <- cbind(all.data.x[source.id, -1], rdf.data.x[source.id, -1])
source.model.name.list  <- c("RF", "LM", "NN")  #"LM", "RF", "NN"
source.itr         <- 1
nbo                <- 100
...
for (source.model.name in source.model.name.list){
  for (itr in 1:source.itr){
    ...
    if (source.model.name == "LM"){
      ...
      source.y.pred <- predict(source.model, as.data.frame(source.x.test))$predict
