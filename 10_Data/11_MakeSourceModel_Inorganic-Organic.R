...
##Setting
prop.mat <- t(matrix(c(1, 292, "Inorg_BandGap",
                       1, 294, "Inorg_DielectricConstant",
                       1, 296, "Inorg_RefractiveIndex",
                       2, 296, "Polym_BandGap",
                       2, 298, "Polym_DielectricConstant",
                       2, 301, "Polym_AtomizationEnergy",
                       2, 302, "Polym_Density",
                       2, 303, "Polym_RefractiveIndex",
                       1, 297, "Inorg_Density",
                       1, 298, "Inorg_Volume",
                       2, 295, "Polym_Volume"
),
...
##Main
for (i.prop in c(1:11)){
  data.flg      <- as.numeric(prop.mat[i.prop, 1])
  source.y.area <- as.numeric(prop.mat[i.prop, 2])
  source.y.name <- prop.mat[i.prop, 3]
  if (data.flg == 1){
    source.data <- read.csv("../10_Data/inorganic_merge_v2.csv")
  } else {
    source.data <- read.csv("../10_Data/organic_merge.csv")
  }
    ...
    for (itr in source.itr.list){
      ...
      if (source.model.name == "LM"){
        ...
        for (nl in 1:num.layer){
          tmp.n          <- max(min(max(2,ceiling(runif(1,min=0.8,max=0.9)*tmp.n)),200),10)
          num.hidden[nl] <- tmp.n
        }
        #num.hidden <- c(200, 150, 100, 50, 10, 1)
