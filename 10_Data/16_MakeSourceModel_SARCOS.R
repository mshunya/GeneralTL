...
##Setting
prop.mat <- t(matrix(c(22, "1st-torque",
                       23, "2nd-torque",
                       24, "3rd-torque",
                       25, "4th-torque",
                       26, "5th-torque",
...
source.train            <- read.csv("../10_Data/SARCOS_train.csv" ,header=FALSE)
source.test             <- read.csv("../10_Data/SARCOS_test.csv" ,header=FALSE)
source.x.area           <- c(1:21)
...
alpha.list  <- c(0, 1)
lambda.list <- c(0, 1)
ntree.list  <- c(10L, 200L)
mtry.list   <- c(2L,  15L)

##Main
for (i.prop in c(1:7)){
  source.y.area <- as.numeric(prop.mat[i.prop, 1])
  source.y.name <- prop.mat[i.prop, 2]
  
  source.x.train <- as.matrix(source.train[, source.x.area])
  source.y.train <- as.vector(source.train[, source.y.area])
  source.x.test  <- as.matrix(source.test[,  source.x.area])
  source.y.test  <- as.vector(source.test[,  source.y.area])
  
  tmp.id <- !is.na(source.y.train)
  source.y.train <- source.y.train[tmp.id]
  source.x.train <- source.x.train[tmp.id,]
  
  tmp.id <- !is.na(source.y.test)
  source.y.test <- source.y.test[tmp.id]
  source.x.test <- source.x.test[tmp.id,]
  ...
  for (source.model.name in source.model.name.list){
    for (itr in source.itr.list){
      ...
      ##Data
      num.source <- dim(source.x.train)[1]
      ...
      if (source.model.name == "LM"){
        ...
        for (nl in 1:num.layer){
          tmp.n          <- max(min(max(2,ceiling(runif(1,min=0.8,max=0.9)*tmp.n)),20),3)
          num.hidden[nl] <- tmp.n
        }
        num.hidden <- c(20, 20, 10, 5, 1)
